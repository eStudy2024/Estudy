...
impl From<PoolError> for AppError {
    fn from(_err: PoolError) -> Self {
        AppError::InternalServerError(json!({"error": "Unknown error at Database poll"}))
    }
}

impl From<BcryptError> for AppError {
    fn from(_err: BcryptError) -> Self {
        AppError::InternalServerError(json!({"error": "Have a problem when decrypting password"}))
    }
}
...
impl From<R2D2Error> for AppError {
    fn from(_err: R2D2Error) -> Self {
        AppError::InternalServerError(json!({"error": "R2D2 error!"}))
    }
}
        ...
        match err {
            DieselError::DatabaseError(kind, info) => {
                if let DatabaseErrorKind::UniqueViolation = kind {
                    ...
                    AppError::InternalServerError(json!({"error": "Unknown error when handling Database"}))
                }
            }
            DieselError::NotFound => {
                AppError::NotFound(json!({ "error": "requested record was not found" }))
            }
            _ => AppError::InternalServerError(json!({"error": "Run out of diesel"})),
        }
    }
}
